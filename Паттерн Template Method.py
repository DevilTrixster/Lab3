...
"""
Вопрос: Как вы можете расширить систему, добавив новый тип заказа (например, заказ с предоплатой)? 
Какие изменения потребуются в коде? Также, как вы реализуете функционал для обработки заказов 
с предоплатой в методе ProcessOrder()?
...
- Легко добавлять новые типы обработки заказов
- Общий алгоритм защищен от изменений
- Повторное использование кода через наследование

Пример дополнительного расширения - заказ с подпиской:

class SubscriptionOrderProcessing(OrderProcessing):
    def process_payment(self, order):
        print(f"Оформлена подписка на сумму {order.total_amount} руб./мес")

    def arrange_delivery(self, order):
        print("Организована регулярная доставка по подписке")

    def send_confirmation(self, order):
        super().send_confirmation(order)
        print("Отправлен календарь доставок")
"""