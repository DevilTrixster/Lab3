...
"""
Вопрос: Как вы будете расширять систему, добавляя новые типы расчетов (например, скидки)? 
...
- Легко добавлять новые операции без изменения структуры объектов
- Возможность комбинировать различные расчеты
- Четкое разделение логики разных расчетов

Дополнительные типы расчетов, которые можно добавить:
- InsuranceCalculator (страховка)
- EnvironmentalFeeCalculator (экологический сбор)
- ImportDutyCalculator (таможенные пошлины)
- LoyaltyPointsCalculator (баллы лояльности)

Ограничения:
- При добавлении новых типов компонентов в иерархию потребуется изменить интерфейс Visitor
- Может быть избыточным для простых систем с малым количеством операций
"""